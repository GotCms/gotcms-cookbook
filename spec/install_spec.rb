...
describe 'gotcms::install' do
  ...
  describe 'Normal execution' do
    ...
    it 'send http post to lang' do
      expect(chef_run).to post_gotcms_request('lang')
        .with(url: 'http://fauxhai.local/install',
              message: 'lang=en_GB',
    ...
    it 'send http post to license' do
      expect(chef_run).to post_gotcms_request('license')
        .with(url: 'http://fauxhai.local/install/license',
    ...
    it 'send http post to check server configuration' do
      expect(chef_run).to post_gotcms_request('check-config')
        .with(url: 'http://fauxhai.local/install/check-server-configuration',
    ...
    it 'send http post to check database configuration' do
      expect(chef_run).to post_gotcms_request('check-database')
        .with(url: 'http://fauxhai.local/install/database-configuration',
              message: 'dbname=gotcmsdb&driver=pdo_mysql&hostname=127.0.0.1&password=gotcmspassword&username=gotcmsuser',
              headers: { 'Cookie' => 'PHPSESSID=installgotcms', 'Content-Type' => 'application/x-www-form-urlencoded' },
              should_redirect: '/install/configuration')
    end

    it 'send http post to configure the cms' do
      expect(chef_run).to post_gotcms_request('configure')
        .with(url: 'http://fauxhai.local/install/configuration',
  ...
  describe 'Override attributes' do
    ...
    it 'send ajax requests to complete install' do
      %w(c-db i-d i-t c-uar it).each do |step|
        expect(chef_run).to post_gotcms_request("complete-step-#{step}")
          .with(url: 'http://fauxhai.local/install/complete',
      ...
      expect(chef_run).to post_gotcms_request('complete-step-c-cf')
        .with(url: 'http://fauxhai.local/install/complete',
